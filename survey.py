...
class SurveySystem():
    ...
    def add_question(self, q_text, type, required, responses,survey_id):
        ...
        if not type in range(1, 3):
            return 0
        newQ = Question(text=q_text, state=1, type=type,
                        required=required, responses=str(responses), survey_id=survey_id)
        db.session.add(newQ)
        db.session.commit()
        return 1
    ...
    def create_survey(self, name, s_id, questions):

        if name.isspace() or name == "" or not questions:
            return 0

        newS = Survey(id = s_id,name=name, state=1, questions=str(questions),uid = session['user_id'],access=1)
        db.session.add(newS)
        db.session.commit()
        return 1
    ...
    def create_user(self, uid, username, password, type=1):
        ...
        newU = User(id=uid, username=str.lower(username),
                    password=generate_password_hash(password), type=type)
        db.session.add(newU)
        db.session.commit()
        return 1
    ...
    #     else:
    #         enrolments = Enrolment.query.filter_by(
    #             u_id=session['user_id']).all()
    #         course_ids = [r[0] for r in Enrolment.query.filter_by(
    #             u_id=session['user_id']).with_entities(Enrolment.c_id).all()]
    ...
    #                 name=row[1], session=row[2]).first()
    #             if course:
    #                 c_id = course.id
    #             else:
    #                 return 0
    ...
    #                 user = User(id=row[0], username=row[0], password=generate_password_hash(
    #                     row[1]), type=u_type)
    #                 db.session.add(user)
